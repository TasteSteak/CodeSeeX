...
此版本优化了原生 Responses 链路的可用性与稳定性：让已启用的图像生成在原生链路中真正执行，并在 provider 边界统一适配官方 DeepSeek 契约，修复了旧版本中思考模式回放、工具输出顺序和内联图片体积这三类会让整轮被拒的问题。客户端契约与显示保持不变。

### 原生工具

- 已启用的 CodeSeeX 托管工具（当前为图像生成）现在会在原生 Responses 链路里声明并由 CodeSeeX 直接执行，不再交回给没有对应实现的客户端；未启用时仍按客户端的声明原样透传，工具归属不静默改变。

### 原生适配

- 思考模式下回放带工具调用的回合时保留该轮自己的 `reasoning_text`。此前托管工具的留档只保留调用本身，reasoning 被丢弃，上游会以 `The reasoning_text in the thinking mode must be passed back to the API.` 拒绝整轮。
- 转发前把同一轮的工具输出收拢成连续一组，并按调用顺序排列。Codex 会在并行工具输出之间插入自己的通知（例如图片缩放提示），上游因此判定后面的调用没有输出并返回 `No tool output found for tool call ...`；被移动的只是这些通知的先后位置，内容不变。
- 回放时收敛内联图片体积：Codex 会把工具返回的图片以 base64 留在历史里，长会话每轮都会重传同一批数据，上游前置代理会直接以 `413 Request Entity Too Large` 拒绝整个请求。CodeSeeX 只在上游那份副本里从最旧的图片开始替换为有界文本标记，最新的图片保持可见，客户端自己的历史与显示不变，被裁剪时会写入日志。

### 本地搜索

- 修正网页正文选取时短代码块可能被重复入选的问题，该缺陷会让选取阶段以越界切片崩溃并中断整次搜索，现在每个正文块只会入选一次。

## 0.8.1 - 2026-09-14

此版本重写了 CodeSeeX 本地 Web Search 的整条链路：从请求到证据收敛为一份结果契约，由模型视图与客户端视图共用，并修复了旧实现把搜索藏起来、中文查询检索不到、页面文本浪费以及续接请求被拒等问题。

### 本地搜索

- 链路按「请求 → 搜索源 → 融合排序 → 抓页 → 正文提取 → 结果」重新分层，模型视图与客户端视图共用同一份结果契约，不再各自维护一套几乎相同的字段。
- CodeSeeX 自己执行的搜索现在会作为标准 `web_search_call` 条目回给客户端，用户能看到搜索词与搜索步骤，而不是只看到最终答案。
- 该条目采用上游回放时认可的字段形态，且不会回传给上游，续接请求不会再因为它被拒绝。
- 混合工具组不再让整轮失败：CodeSeeX 只执行属于自己的一侧，客户端工具照常归还，整轮作为一组完成回放。
- 新增本机管理接口 `POST /api/web-search/probe`，可脱离模型直接跑一次搜索或打开一页，便于排查。

### 搜索质量

- 请求只携带查询词，不注入 market、language 或 locale 参数，来源保持地区中立。
- 跨来源改为按倒数排名融合，取代「先到先得」，排序不再取决于哪个响应先返回，同一网址被多源命中会获得更高位次。
- 无空格文字按二元切分参与匹配，中文等查询不再因为要求整段原样命中而检索不到。
- 候选需通过相关性门禁：搜索结果页里与查询无关的条目会被丢弃，而不是塞给模型。
- 来源健康区分「不可达」与「可达但无结果」；仅在连续失败后退避，同一来源的请求按最小间隔节流，查询结果缓存十分钟，探活要求真正解析出结果。

### 正文与证据

- 页面按真实 DOM 解析成块并逐块打分，给出正文占比、链接占比与置信度，取代整页压成一条纯文本。
- 代码块保留换行与缩进；表格行按列读成一行；块的链接目标解析为绝对地址一并给出。
- 超长页面按预算挑选开头、最高分与命中查询的块，并对省略部分给出显式账目，不再从头部截断。
- 页内重复出现的块只保留首次，模板内容不再重复占用预算。
- 资源载荷不进入模型：整段 `data:` URL 与超过体量阈值的不透明串会被替换为有界标记，图片与媒体只保留引用。判定只看体量与标签，不针对任何站点或语言。
- 证据预算即模型可见预算，管线选中的内容不会被再次截掉。

### 会话与日志

- 续接请求不再因为回传的搜索条目缺少上游必需字段而被拒绝。
- 代理不代跑客户端自己的工具，客户端工具输出不受搜索条目处理的影响。
- 抓页保留地址固定、逐跳重定向校验与私网拦截，并补齐更多保留网段。
- 本地搜索的日志只保留用户级摘要；上下文成本提示仅在真正超过尺寸阈值时记录，不再每轮写入。

## 0.8.0 - 2026-09-13

此版本优化了大量底层实现：把上游地址、模型清单、定价与工具归属收敛为单一数据来源，并按常驻客户端的形态重新定调传输方式、日志与反馈；优化了配置编辑、模型选择、用量查看与故障排查体验；并修复了旧版本遗留的上游静默回落、用量归集断链、原生工具声明被拒、apply_patch 兼容与未定价静默计费等问题。

### 模型与定价

- 模型清单、别名、能力与定价合并为一份带版本的目录文档，按「用户覆盖 → 远程清单 → 本地缓存 → 内置文档」分层解析，不发新客户端也能更新模型与价格。
- 远程清单在启动、每 6 小时与手动刷新时复查；任何失败都保留上一层数据，且不阻塞代理。
- 公开清单迁移到 `catalog/model-catalog.json`，写入真实 DeepSeek 模型与官方人民币价格。
- 新增 `kind`（`chat` / `billing_only`）与语义化 `role`（`chat` / `vision`）：识图模型照常计价、照常参与回放，但不进入选择、上锁与托盘。
- 定价按精确 slug → 费率分组 → 显式未定价解析；峰谷窗口与倍数由 store 与前端共用一份实现。

### 上游与传输

- 上游地址改由 Codex 的 `config.toml` 承载（`[codeseex] upstream_base_url`），设置页直接编辑该文件，且只在用户真正设置过时才写入该键。
- 原生 Responses 成为默认且唯一的隐式传输方式，`auto` 模式移除。
- 上游凭据来源显式化（`auto` / `request` / `env` / `codex_auth` / `secret`），客户端身份头原样透传，中转不再因缺少 Codex 身份而被拒绝。
- 新增目录层管理接口：`GET /api/catalog`、`POST /api/catalog/refresh`、`GET /api/upstream/probe`、`POST /api/upstream/test`、`POST /api/upstream/credential`。

### 代理运行时

- apply_patch 输入在两条链路上统一规范化：旧式 unified 范围头改为裸 `@@`，并修复只含空格的空上下文行；修复结果写入 `apply_patch_input_micro_repair_diagnostic` 诊断事件。
- 原生 Responses 续传不再因分组工具声明、Codex App 重启后的重复 namespace、或仅由顶层 union 构成的工具 schema 而失败，修复过程记录到事件日志。
- 日志分为「用户 / 调试」两档：默认只保留生命周期与失败，调试档附带每轮簿记；事件接口默认返回面向用户的部分。
- Chat 兼容流的用量扫描加上与原生中继一致的上限，未分帧的上游不再让缓冲区无限增长。

### 工具

- 工具归属只保留一个权威来源：内置工具名单、别名归一化与「由谁执行」的判定统一收敛，不再在各处另抄一份。
- 上游原生 `web_search` 声明改为在原生传输内部执行，不再为此切换到 Chat 兼容通道，工具归属不会被静默改变。
- 工作区范围与全权限标记只信任 Codex 注入的上下文，不再采信工具输出。

### 用量与日志

- 一个用户回合对应一个用量会话：原生工具交接带标记，中间回复留在所属回合内，失败轮次显示为所属回合内的失败行。
- 用量行由全部保留请求生成，仅裁剪对外历史，长回合不再丢失早期轮次；分组一次遍历完成，会话修订号改为小哈希。
- 仪表盘新增滚动 60 秒的 RPM 与 TPM，以及平均延迟。

### 客户端界面

- 设置页重组为「客户端（个性化、其它）/ 代理（连接、模型行为）/ 工具」，网页搜索后端移入网页搜索工具卡片。
- 新增 toast 管理器：正上方居中、整条可点击消失、同屏最多 3 条其余排队、同 key 复用并累加 `×N`、队列满后合并为一条摘要。
- 模型卡片支持上锁固定上游；模型列表最多 3 行并支持滚动。
- 思考链镜像模式：`none` / `smart` / `fixed` / `full`，可独立配置且不改变发给上游模型的内容。

### Codex App 集成

- 「启动 Codex」提供无注入与注入两种模式：无注入完全不触碰渲染器；注入可随时移除，无需重启 Codex。
- 托盘模型菜单跟随当前目录，不再固定内置 slug。

### 迁移与兼容

- CodeSeeX 侧的 `[upstream] base_url` 已移除：首次加载时会迁移为 Codex `config.toml` 里的 `[codeseex] upstream_base_url`，此后以 Codex 文件为准；`DEEPSEEK_BASE_URL` 仍可覆盖两者。
- 旧文件里的 `transport = "auto"` 仍可加载（解析为原生 Responses），但不会再被写出；未设置即默认。
- 已移除的 `[experimental] reasoning_summary` 布尔键会被忽略而非翻译，模式键为准，默认 `smart`。
- `[catalog] mode` 会被忽略；`[model] override = "flash" | "pro"` 与 `BILLING_*` 键仍可读取。
