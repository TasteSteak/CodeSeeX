...
CodeSeeX 0.8.0 把上游地址搬进 Codex 自己的配置，让目录成为模型信息的唯一来源，并把常驻运行时的表面收敛到长期客户端真正需要展示的内容。本条目汇总整条 0.7.0 → 0.8.0 线，包含 0.7.1 首次发布的目录与定价基础。

### 重点更新

- 上游地址改为存放在 Codex 的 `config.toml`：`[codeseex] upstream_base_url`。设置页直接编辑该文件，运行时按需解析（所有入口保持一致），0.8.0 之前写在 CodeSeeX 侧的值会自动迁移一次，且生成的 Codex TOML 只在用户真正设置过时才写入该键。
- 模型清单、别名、能力与定价收敛为一份带版本的目录文档，按层解析（用户覆盖 → 远程清单 → 本地缓存 → 内置文档）。公开清单移到 `catalog/model-catalog.json`，并写入真实的 DeepSeek 模型与官方人民币价格。
- 目录条目可以是 `billing_only`：照常计价、照常参与回放解析，但不会出现在聊天选择、上锁或托盘里。
- 原生 Responses 成为默认且唯一的隐式传输方式，`auto` 模式被移除。
- 日志台默认只显示生命周期与失败；设置页新增「用户 / 调试」开关按需恢复每轮诊断，瞬时反馈改由带队列的 toast 管理器承载。
- 一个用户回合对应一个用量会话：原生工具的交接会被标记，中间回复留在所属回合内；仪表盘新增实时 RPM / TPM 与平均延迟。

### 新增

- toast 管理器：正上方居中、整条可点击消失、同屏最多 3 条其余排队、同 key 复用并累加 `×N` 计数、队列满后合并为一条摘要。
- 设置 → 客户端 → 其它 新增日志详细度开关：`用户` 只保留必要消息，`调试` 附带每轮簿记信息；事件接口默认返回面向用户的部分。
- 仪表盘吞吐卡片：滚动 60 秒的 RPM 与 TPM，以及平均延迟。
- 目录新增 `kind`（`chat` | `billing_only`）与语义化 `role`（`chat` | `vision`），识图模型等能力默认值来自数据而非代码里的 slug。
- 思考链镜像模式：`none`、`smart`、`fixed`、`full`，可独立配置且不改变发给上游模型的内容。
- 模型卡片支持上锁：悬浮显示锁按钮，可将该目录模型固定为上游，再次点击恢复跟随客户端。
- 托盘模型菜单跟随当前目录，而不再固定内置 slug。
- 目录层管理接口：`GET /api/catalog`、`POST /api/catalog/refresh`、`GET /api/upstream/probe`、`POST /api/upstream/test`、`POST /api/upstream/credential`。

### 变更

- 上游地址来源从 CodeSeeX 自己的 `config.toml` 迁到 Codex 的 `config.toml`（`[codeseex] upstream_base_url`），设置页成为该文件的编辑器。
- 别名与出站 slug 处理完全由目录驱动，删除写死的 `gpt-5*` 映射。
- 设置页重组为「客户端（个性化、其它）/ 代理（连接、模型行为）/ 工具」，网页搜索后端移入网页搜索工具卡片。
- 日志台不再逐条叙述每次往返：请求级与轮次级簿记归入诊断受众，而警告与错误始终显示。
- 用量行改由全部保留请求生成，仅裁剪对外历史，长回合不再丢失早期轮次；分组一次遍历完成，会话修订号改为小哈希。
- 模型列表最多 3 行并支持滚动；`billing_only` 条目不参与选择、上锁与托盘。
- 定价按精确 slug → 费率分组 → 显式未定价解析；峰谷窗口与倍数在 store 与前端共用一份实现。
- 客户端身份请求头原样透传，上游凭据来源显式化（`auto` / `request` / `env` / `codex_auth` / `secret`）。

### 修复

- 桌面端内嵌代理静默回落到官方 DeepSeek 端点，导致中转 key 每次请求都 401。环境级基础配置构造现已私有，Codex 侧覆盖按需解析，旧值自动迁移。
- 图像理解改为跟随已配置上游并共用凭据，透传客户端身份使中转不再拒绝，默认模型来自目录。
- 用量归集保持回合完整：标记原生工具交接使一个回合对应一个会话，超过旧时间窗的长回合也能匹配自身交接，失败轮次留在所属回合内显示为失败行。
- 工作区范围与全权限标记只信任 Codex 注入的上下文，不再采信工具输出；日志摘要与模型可见路径使用同一套脱敏；凭据库「读取失败」不再被当作「未配置」。
- Chat 兼容流的用量扫描加上与原生中继一致的上限，未分帧的上游不再让缓冲区无限增长。
- 目录来源标签如实报告（`builtin` / `cache` / `remote`）；`[model] thinking` 只解析一次；空模型请求在所有路径都解析为目录默认模型。
- 原生 Responses 续传不再因分组工具声明、Codex App 重启后的重复 namespace、或仅由顶层 union 构成的工具 schema 而失败，修复过程记录到事件日志。
- 未定价模型不再被静默按 Pro 价格计费，设置页会明确标注「未定价」。
